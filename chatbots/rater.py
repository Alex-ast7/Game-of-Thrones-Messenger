...
class Rater(ChatBot):
    ...
    def answer_load(self):
        try:
            ...
            output = self.model(**self.tokenizer(self.context, max_length=512, return_tensors='pt', truncation=True))
            vector = []
            ...
            for row in vectors.iterrows():
                distance = cosine_similarity([list(row[1])[1:]], [vector])[0][0]
                if distance > min_d:
                    min_d = distance
                    min_id = list(row[1])[0]
            con = sqlite3.connect('db/messenger.db')
            cur = con.cursor()
            answer = cur.execute(f'select Название from Episodes where id={min_id}').fetchone()[0]
            con.close()
            self.progress.emit(100)
